...
void PackInstallTask::executeTask()
{
    ...
    netJob->addNetAction(Net::Download::makeByteArray(QUrl(searchUrl), m_response_ptr));
...
void PackInstallTask::resolveMods()
{
    ...
    for (auto const& file : m_version.files) {
        if (!file.serverOnly && file.url.isEmpty()) {
            ...
            flame_file.version.hash = file.sha1;
...
void PackInstallTask::onResolveModsSucceeded()
{
    ...
    for (int index = 0; index < m_file_id_map.size(); index++) {
        ...
        // czm: no guarantees
        if (!results_file.pack.versionsLoaded || results_file.version.downloadUrl.isEmpty()) {
